...
export const getFormFieldTitle = (field: UiNodeInputAttributes): string =>
  field.name && field.name in translations
    ? translations[field.name as keyof Translations].title
    : field.name;

export const getFormFieldPosition = (field: UiNodeInputAttributes): number =>
  field.name && field.name in translations
    ? translations[field.name as keyof Translations].position
    : Infinity;

export const getFormPlaceholder = (field: UiNodeInputAttributes): string =>
  field.name && field.name in translations
